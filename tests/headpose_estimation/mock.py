...
class MockBackBoneModel(PretrainedBackBoneImageModel):
  ...
  def forward(self, input_data: List[bytes]) -> List[np.ndarray]:
    preprocessed_image_input: List[np.ndarray] = self._preprocess_raw_images(input_Images=input_data)
    ...
    image_representation = self.session.run(
      ...
      feed_dict={self.pretrained_img_model_input_node: batched_preprocessed_images},
    )
