...
class TestBasePretrainedImageModel:
  ...
  def test__preprocess_raw_images(
    ...
    output: List[np.ndarray] = backbone_model._preprocess_raw_images(input_images=input_images)
    resized_image_size = (
      1,
...
class TestEulerAnglesPredictionHead:
  ...
  def test_forward(
    ...
    input = np.random.uniform(size=[self.BATCH_SIZE, self.TEST_INPUT_SHAPE]).astype(np.float32)
