...
class Image_Dataset:
	...
	def create_list(self):
		...
		for extension in extensions:
			file_glob = os.path.join(self.image_dir, '*.' + extension)
		...
		with open(self.gt_file, 'r') as csvfile:
			...
			for row in label_file:
				labels.append([float(values) for values in row])
...
class Bottlenecks:
	...
	def run_on_image(self, image_data):
		resized_input_values = self.sess.run(self.decoded_image_tensor,
											feed_dict={self.jpeg_data_tensor:image_data})
		value = self.sess.run(self.bottleneck_tensor,
								feed_dict={self.resized_input_tensor:resized_input_values})
	...
	def get_rand_cached(self, how_many, category):
		...
		if how_many >= 0:
			...
			for unused_i in range(how_many):
				...
				bottlenecks.append(self.get_or_create(image_index,
														category))
				ground_truth.append(self.image_dataset.get_ground_truth(image_index,
																		category))
		else:
...
class Model:
	def __init__(self, flags):
		self.flags = flags
		self.dest_directory = flags.model_dir
		self.model_info = {}
		self.graph = None
		...
		self.data_url = self.model_info['data_url']
		self.filename = self.data_url.split('/')[-1]
		self.filepath = os.path.join(self.dest_directory, self.filename)

		self.download_extract_if_needed()
		[self.graph, self.bottleneck_tensor, self.resized_input_values] = self.create_model_graph()

	def create_dest_dir(self, directory):
	...
	def download(self):
		def _progress(count, block_size, total_size):
			...
			sys.stdout.flush()
			filepath, _ = urllib.request.urlretrieve(self.data_url, self.filepath, _progress) #### filepath clobbered !!!
	...
	def extract(self):
		...
		tarfile.open(self.filepath, 'r:gz').extractall(self.dest_directory)
	...
	def create_model_info(self):
		...
		if architecture == 'inception_v3':
			...
			if (version_string != '1.0' and version_string != '0.75' and
				...
				tf.logging.error(""""The Mobilenet version should be '1.0',
					'0.75', '0.5', or '0.25', but found '%s' for architecture '%s'""", version_string, architecture)
				return None

			size_string = parts[2]
			if (size_string != '224' and size_string != '192' and
				size_string != '160' and size_string != '128'):
				tf.logging.error("""The Mobilenet input size should be '224',
					'192', '160', or '128', but found '%s' for architecture '%s'""", size_string, architecture)
				return None
	...
	def add_final_retrain_ops(self, is_training=True):
		...
		with tf.name_scope('input'):
			...
			roll_ground_truth_input = tf.placeholder(tf.float32, shape=[None, 1], name='roll_GroundTruthInput')

			yaw_ground_truth_input = tf.placeholder(tf.float32, shape=[None, 1], name='yaw_GroundTruthInput')

			pitch_ground_truth_input = tf.placeholder(tf.float32, shape=[None, 1], name='pitch_GroundTruthInput')

			ground_truth_input = [roll_ground_truth_input, yaw_ground_truth_input, pitch_ground_truth_input]

		with tf.name_scope('final_retrain_ops'):
			with tf.name_scope('weights'):
				roll_initial_value = tf.truncated_normal([bottleneck_tensor_size, 1], stddev=0.001)
				pitch_initial_value = tf.truncated_normal([bottleneck_tensor_size, 1], stddev=0.001)
				yaw_initial_value = tf.truncated_normal([bottleneck_tensor_size, 1], stddev=0.001)

				roll_layer_weights = tf.Variable(roll_initial_value, name='roll_final_weights')
				pitch_layer_weights = tf.Variable(pitch_initial_value, name='pitch_final_weights')
				yaw_layer_weights = tf.Variable(yaw_initial_value, name='yaw_final_weights')
			...
			with tf.name_scope('Yaw_MSE'):
				yaw_MSE_loss = tf.losses.mean_squared_error(labels=yaw_ground_truth_input, predictions=yaw_logits)
				tf.summary.scalar('Yaw_Error', yaw_MSE_loss)

			with tf.name_scope('Pitch_MSE'):
				pitch_MSE_loss = tf.losses.mean_squared_error(labels=pitch_ground_truth_input, predictions=pitch_logits)
				tf.summary.scalar('Pitch_Error', pitch_MSE_loss)

			with tf.name_scope('Roll_MSE'):
				roll_MSE_loss = tf.losses.mean_squared_error(labels=roll_ground_truth_input, predictions=roll_logits)
	...
	def run_final_eval(self):
		sess, bottleneck_input, ground_truth_input, last_layer, streaming_correlation, final_correlations = self.build_eval_sess()

		test_bottlenecks, test_ground_truths = self.bottlenecks.get_rand_cached(self.flags.test_batch_size, 'testing')

		test_correlation_values = sess.run(last_layer,
...
def main(_):
	...
	with tf.Session(graph=model.graph) as sess:
		...
		model.bottlenecks = Bottlenecks(sess, images, model.flags.bottleneck_dir, model.flags.architecture, jpeg_data_tensor,
		...
		for i in range(FLAGS.how_many_training_steps):
			...
			if (i % FLAGS.eval_step_interval == 0) or is_last_step:
				yaw_correlation, roll_correlation, pitch_correlation, yaw_loss, roll_loss, pitch_loss = sess.run([streaming_correlations, MSE_loss],
					feed_dict={bottleneck_input:train_bottlenecks,
					ground_truth_input[0]: train_ground_truth[:, 0].reshape(-1, 1),
					ground_truth_input[1]: train_ground_truth[:, 1].reshape(-1, 1),
					ground_truth_input[2]: train_ground_truth[:, 2].reshape(-1, 1)})
				...
				validation_summary, correlation_values = sess.run([merged, streaming_correlations],
		...
		final_answers, rotation_vals = sess.run([final_correlations, last_layer],
			feed_dict={bottleneck_input:test_bottlenecks,
			ground_truth_input[0]: test_ground_truths[:, 0].reshape(-1, 1),
			ground_truth_input[1]: test_ground_truths[:, 1].reshape(-1, 1),
			ground_truth_input[2]: test_ground_truths[:, 2].reshape(-1, 1)})
