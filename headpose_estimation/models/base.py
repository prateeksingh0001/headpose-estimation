...
class BaseAnglePredictionHeadModel(BaseModel[np.ndarray, Dict[str, float]]):
  ...
  def predict(self, session: tf.compat.v1.Session, prediction_input: List[np.ndarray]) -> List[Dict[str, float]]:
    ...
    for i in range(len(prediction_input)):
      output.append(
        {
          self.YAW_ANGLE_KEY: yaw_angle[i][0] * self.EULER_ANGLE_NORMALIZATION_FACTOR,
          self.ROLL_ANGLE_KEY: roll_angle[i][0] * self.EULER_ANGLE_NORMALIZATION_FACTOR,
          self.PITCH_ANGLE_KEY: pitch_angle[i][0] * self.EULER_ANGLE_NORMALIZATION_FACTOR,
        }
      )
