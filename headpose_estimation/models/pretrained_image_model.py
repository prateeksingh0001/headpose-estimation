...
class PretrainedBackBoneImageModel(BaseModel[str, np.ndarray]):
  ...
  def __init__(self, tf_model_config: TensorflowV1ModelConfig) -> None:
    ...
    self._input_placeholder = tf.compat.v1.placeholder(tf.string, shape=[], name="input_image_bytes")
    self._image_preprocessing_output_node = self._create_preprocessing_graph(
      input_raw_image_tensor=self._input_placeholder
    )

    self._pretrained_img_model_input_node, self._prediction_op = self._create_model_graph(
  ...
  def _create_preprocessing_graph(self, input_raw_image_tensor: tf.Tensor) -> tf.Tensor:
    ...
    image_data = tf.expand_dims(decode_jpeg(input_raw_image_tensor, channels=3), axis=0)
    resized_image = resize_bilinear(images=image_data, size=self.model_config.image_input_size)
    scaled_image_data = (resized_image - self.PIXEL_NORMALIZATION_MEAN) * self.PIXEL_NORMALIZATION_MULTIPLIER
  ...
  def _create_model_graph(
    ...
    input_tensor, output_tensor = tf.import_graph_def(graph_def, return_elements=[input_node_name, output_node_name])
  ...
  def predict(self, session: tf.compat.v1.Session, prediction_input: List[str]) -> List[np.ndarray]:
    ...
    if self.model_config.supports_batching:
      ...
      image_representation = session.run(
        ...
        feed_dict={self._pretrained_img_model_input_node: batched_preprocessed_images},
      )
      output_representation = np.vsplit(image_representation, len(image_representation))
