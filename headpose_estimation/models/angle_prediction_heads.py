...
class HopeNetPredictionHead(BaseAnglePredictionHeadModel):
    ...
    def __init__(
        ...
        yaw_angle_logits, yaw_angle_prediction = (
            self._initialize_weights_for_angle_prediction(
                input_image_representation_tensor=self._input_placeholder,
                layer_sizes=layer_sizes,
                name="yaw_predictor",
        ...
        pitch_angle_logits, pitch_angle_prediction = (
            self._initialize_weights_for_angle_prediction(
                input_image_representation_tensor=self._input_placeholder,
                layer_sizes=layer_sizes,
                name="pitch_predictor",
        ...
        roll_angle_logits, roll_angle_prediction = (
            self._initialize_weights_for_angle_prediction(
                input_image_representation_tensor=self._input_placeholder,
                layer_sizes=layer_sizes,
                name="roll_predictor",
    ...
    def _initialize_weights_for_angle_prediction(
        ...
        with tf.name_scope(name):
            ...
            softmax_logits = softmax(
                previous_layer_output, axis=-1, name=f"{name}/softmax_output"
            )
            angle_prediction = (
                tf.reduce_sum(
    ...
    def _calculate_loss_for_angle(
        ...
        angle_bucket_loss = softmax_cross_entropy_with_logits(
            labels=angle_class_gt_tensor, logits=logit_tensor
        )
    ...
    def _initialize_training_ops(
        ...
        yaw_angle_ground_truth_placeholder = tf.compat.v1.placeholder(
            tf.float32, shape=[None], name="input_yaw_gt"
        )
        ...
        roll_angle_ground_truth_placeholder = tf.compat.v1.placeholder(
            tf.float32, shape=[None], name="input_roll_gt"
        )
        ...
        pitch_angle_ground_truth_placeholder = tf.compat.v1.placeholder(
            tf.float32, shape=[None], name="input_pitch_gt"
        )
    ...
    def train(
        ...
        angle_bin_ground_truths = {
            self.YAW_CLASS_KEY: [
                (angle + self.MAX_EULER_ANGLE) / self.ANGLE_PER_BUCKET
                for angle in ground_truths[self.YAW_ANGLE_KEY]
            ],
            self.PITCH_CLASS_KEY: [
                (angle + self.MAX_EULER_ANGLE) / self.ANGLE_PER_BUCKET
                for angle in ground_truths[self.PITCH_ANGLE_KEY]
            ],
            self.ROLL_CLASS_KEY: [
                (angle + self.MAX_EULER_ANGLE) / self.ANGLE_PER_BUCKET
                for angle in ground_truths[self.ROLL_ANGLE_KEY]
            ],
        }
        batched_image_representations = np.vstack(image_representations)
        _, total_loss, yaw_loss, roll_loss, pitch_loss, summary = session.run(
            ...
            feed_dict={
                ...
                self.ground_truth_placeholders[
                    self.YAW_CLASS_KEY
                ]: angle_bin_ground_truths[self.YAW_CLASS_KEY],
                self.ground_truth_placeholders[
                    self.PITCH_CLASS_KEY
                ]: angle_bin_ground_truths[self.PITCH_CLASS_KEY],
                self.ground_truth_placeholders[
                    self.ROLL_CLASS_KEY
                ]: angle_bin_ground_truths[self.ROLL_CLASS_KEY],
            },
        )
