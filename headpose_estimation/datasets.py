...
class BIWI:
    ...
    def get_dataset(self):
        ...
        for frame in self.images:
            cv2.imwrite(
                os.path.join("Images", str(self.frame_count_start) + ".jpg"), frame
            )
    ...
    def get_faces(self, video):
        ...
        for _ in range(300):
            ...
            for idx, det in enumerate(dets):
                ...
                cv2.imwrite(
                    os.path.join("Images", str(self.frame_count_start) + ".jpg"), image
                )
                self.frame_count_start += 1
                break
...
class AFW200:
    ...
    def get_ypr_from_mat(self, mat_path):
        ...
        pose_params = [
            float(pre_pose_params[2]) * (180 / np.pi),
            float(pre_pose_params[0]) * (180 / np.pi),
...
if __name__ == "__main__":
    BIWI_dataset = BIWI(
        data_dir="hpdb", face_model="mmod_human_face_detector.dat", image_mode="RGB"
    )
    ...
    GI4E_dataset = GI4E(
        data_dir="Head_Pose_Database_UPNA",
