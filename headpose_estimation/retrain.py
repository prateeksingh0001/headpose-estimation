...
class Bottlenecks:
    ...
    def run_on_image(self, image_data):
        ...
        value = self.sess.run(
            self.decoded_image_tensor, feed_dict={self.jpeg_data_tensor: image_data}
        )
        output_image = self.sess.run(
            self.bottleneck_tensor, feed_dict={self.resized_input_tensor: value}
        )
    ...
    def create_file(self, path, index, category):
        ...
        try:
            ...
            raise RuntimeError(
                "error during processing file %s (%s)" % (image_path, str(exep))
            )
    ...
    def get_path(self, index, category):
        ...
        base_name = category_list[index]
        full_path = (
            os.path.join(self.bdir, base_name) + "_" + self.architecture + ".txt"
        )
        return full_path
    ...
    def get_rand_cached(self, how_many, category):
        ...
        if how_many >= 0:
            ...
            for unused_i in range(how_many):
                ...
                ground_truth.append(
                    self.image_dataset.get_ground_truth(image_index, category)
                )
        else:
...
class Model:
    ...
    def download(self):
        ...
        filepath, _ = urllib.request.urlretrieve(
            self.model_info["data_url"], self.filepath, _progress
        )  #### filepath clobbered !!!
    ...
    def add_final_retrain_ops(self, is_training=True):
        ...
        with tf.name_scope("input"):
            ...
            ground_truth_input = [
                tf.placeholder(
                    tf.float32, shape=[None, 1], name="roll_GroundTruthInput"
                ),
                tf.placeholder(
                    tf.float32, shape=[None, 1], name="yaw_GroundTruthInput"
                ),
                tf.placeholder(
                    tf.float32, shape=[None, 1], name="pitch_GroundTruthInput"
                ),
            ]

        with tf.name_scope("final_retrain_ops"):
            with tf.name_scope("weights"):
                roll_layer_weights = tf.Variable(
                    tf.truncated_normal([bottleneck_tensor_size, 1], stddev=0.001),
                    name="roll_final_weights",
                )
                pitch_layer_weights = tf.Variable(
                    tf.truncated_normal([bottleneck_tensor_size, 1], stddev=0.001),
                    name="pitch_final_weights",
                )
                yaw_layer_weights = tf.Variable(
                    tf.truncated_normal([bottleneck_tensor_size, 1], stddev=0.001),
            ...
            with tf.name_scope("Wx_plus_b"):
                roll_logits = (
                    tf.matmul(bottleneck_input, roll_layer_weights) + roll_layer_biases
                )
                pitch_logits = (
                    tf.matmul(bottleneck_input, pitch_layer_weights)
                    + pitch_layer_biases
                )
                yaw_logits = (
                    tf.matmul(bottleneck_input, yaw_layer_weights) + yaw_layer_biases
                )
                ...
                last_layer = tf.concat(
                    [roll_logits, yaw_logits, pitch_logits],
                    axis=1,
                    name=FLAGS.final_tensor_name,
                )
            ...
            with tf.name_scope("Yaw_MSE"):
                yaw_mse_loss = tf.losses.mean_squared_error(
                    labels=ground_truth_input[1], predictions=yaw_logits
                )
                tf.summary.scalar("Yaw_Error", yaw_mse_loss)

            with tf.name_scope("Pitch_MSE"):
                pitch_mse_loss = tf.losses.mean_squared_error(
                    labels=ground_truth_input[2], predictions=pitch_logits
                )
                tf.summary.scalar("Pitch_Error", pitch_mse_loss)

            with tf.name_scope("Roll_MSE"):
                roll_mse_loss = tf.losses.mean_squared_error(
                    labels=ground_truth_input[0], predictions=roll_logits
                )
                tf.summary.scalar("Roll_Error", roll_mse_loss)

            with tf.name_scope("train"):
                roll_optimizer = tf.train.AdamOptimizer(FLAGS.learning_rate)
                roll_train_step = roll_optimizer.minimize(
                    roll_mse_loss, var_list=[roll_layer_weights, roll_layer_biases]
                )

                yaw_optimizer = tf.train.AdamOptimizer(FLAGS.learning_rate)
                yaw_train_step = yaw_optimizer.minimize(
                    yaw_mse_loss, var_list=(yaw_layer_weights, yaw_layer_biases)
                )

                pitch_optimizer = tf.train.AdamOptimizer(FLAGS.learning_rate)
                pitch_train_step = pitch_optimizer.minimize(
                    pitch_mse_loss, var_list=(pitch_layer_weights, pitch_layer_biases)
                )
        ...
        with tf.name_scope("correlations"):
            with tf.name_scope("Yaw_correlation"):
                yaw_correlation, yaw_update_op = streaming_pearson_correlation(
                    predictions=result_tensor[:, 0], labels=ground_truth_tensor[0]
                )
            ...
            with tf.name_scope("Roll_correlation"):
                roll_correlation, roll_update_op = streaming_pearson_correlation(
                    predictions=result_tensor[:, 1], labels=ground_truth_tensor[1]
                )
            ...
            with tf.name_scope("Pitch_correlation"):
                pitch_correlation, pitch_update_op = streaming_pearson_correlation(
                    predictions=result_tensor[:, 2], labels=ground_truth_tensor[2]
                )
    ...
    def build_eval_sess(self):
        ...
        with self.eval_graph.as_default():
            (
                ...
                ground_truth_input,
                last_layer,
            ) = self.add_final_retrain_ops(False)

            tf.train.Saver().restore(eval_sess, CHECKPOINT_NAME)
            streaming_correlations, final_correlations = self.add_eval_step(
                last_layer, ground_truth_input
            )
    ...
    def run_final_eval(self):
        (
            ...
            _,
            _,
        ) = self.build_eval_sess()

        test_bottlenecks, test_ground_truths = self.bottlenecks.get_rand_cached(
            self.flags.test_batch_size, "testing"
        )
        ...
        tf.logging.info(
            "Final average test correlation = %.1f" % (average_test_correlation)
        )
...
def main_setup(FLAGS):
    ...
    with model.graph.as_default():
        (
            ...
            ground_truth_input,
            last_layer,
        ) = model.add_final_retrain_ops(is_training=True)
    ...
    with tf.Session(graph=model.graph) as sess:
        ...
        streaming_correlations, final_correlations = model.add_eval_step(
            last_layer, ground_truth_input
        )
        ...
        for i in range(FLAGS.how_many_training_steps):
            ...
            train_bottlenecks, train_ground_truth = model.bottlenecks.get_rand_cached(
            ...
            if (i % FLAGS.eval_step_interval == 0) or is_last_step:
                ...
                tf.logging.info(
                    "%s: Step %d: average loss:%.1f average correlation = %.1f"
                    % (
                        datetime.now(),
                        i,
                        (yaw_loss + roll_loss + pitch_loss) / 3,
                ...
                ) = model.bottlenecks.get_rand_cached(
                ...
                validation_summary, correlation_values = sess.run(
                    ...
                    feed_dict={
                        ...
                        ground_truth_input[0]: validation_ground_truth[:, 0].reshape(
                            -1, 1
                        ),
                        ground_truth_input[1]: validation_ground_truth[:, 1].reshape(
                            -1, 1
                        ),
                        ground_truth_input[2]: validation_ground_truth[:, 2].reshape(
                            -1, 1
                        ),
                    },
                )
                ...
                tf.logging.info(
                    "%s: Step %d: Validation correlation = %1.1f (N=%d)"
                    % (
                        datetime.now(),
                        i,
            ...
            if (
                ...
                intermediate_file_name = (
                    FLAGS.intermediate_output_graph_dir
                    + "intermediate"
                    + str(i)
                    + ".pb"
                )
        ...
        test_bottlenecks, test_ground_truths = model.bottlenecks.get_rand_cached(
