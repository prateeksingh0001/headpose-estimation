...
@dataclass
class SingleDatasetPreprocessingConfig:
  dataset_name: str
...
class DataPreprocessingConfig:
  ...
  output_path: Path
  datasets: List[SingleDatasetPreprocessingConfig]

  @classmethod
  def load_from_config(cls, config_path: str) -> DataPreprocessingConfig:
    with open(config_path, "r") as f:
      config_dict = yaml.safe_load(f)

    datasets = [
      SingleDatasetPreprocessingConfig(**single_dataset_conf) for single_dataset_conf in config_dict["datasets"]
    ]

    return cls(
      dataset_root_path=Path(config_dict["dataset_root_path"]),
...
class Dataset:
  ...
  def shuffle_and_train_test_split(
    ...
    validation_dataset = Dataset(
      datapoints=self.datapoints[training_data_size : (training_data_size + validation_data_size)]
    )
    test_dataset = Dataset(
      datapoints=self.datapoints[
        (training_data_size + validation_data_size) : (training_data_size + validation_data_size + test_data_size)
      ]
    )
...
class ExperimentConfig:
  ...
  def from_yaml(cls, config_path: str) -> ExperimentConfig:
    ...
    prediction_model_config = PredictionHeadModelConfig(**config_dict.get("prediction_head", {}))
    ...
    if args["config_path"]:
      ...
      if "optimizer_class" in args and "learning_rate" in args:
        ...
        if "learning_rate_decay_function" in args:
          optimizer_config.learning_rate_decay_function = args["learning_rate_decay_function"]
      ...
      training_config = TrainingConfig(
        ...
        intermediate_representations_save_dir=args["intermediate_representation_save_dir"],
