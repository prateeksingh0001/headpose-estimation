...
def main() -> None:
    ...
    for key in shuffled_data_keys:
        img_data = loadmat(
            image_name_to_data_paths[key][ground_truth_filepath_identifier]
        )
        try:
            ...
            logger.warning(
                f"Did not find key Pose_Para in {image_name_to_data_paths[key][img_filepath_identifier]}"
            )
    ...
    with open(
        Path(args.output_path).joinpath("train_dataset.csv"), "w"
